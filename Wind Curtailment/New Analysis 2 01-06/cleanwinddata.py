...
def get_curtailment_mw(row):
    return (row['Wind Curtailment'] * row['Wind generation MW'])/100
...
df = pd.read_excel('apwindcurtailment.xlsx')
...
with pd.ExcelWriter('BlockWiseData.xlsx',engine='openpyxl') as writer:
    ...
    for given_date in date_list:
        ...
        df_mw_curtailment_list = [0 for x in range(1,97)] #Cumulative Wind Curtailment MW Column
        for x in range(len(curtailment_list)):
            df_mw_curtailment_list[x] = mw_curtailment_list[x] + mw_temp
